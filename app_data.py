    ...
    def __init__(self):
        ...
        with open(self.schema_path, 'r', encoding='utf-8') as f:
            for line in f.readlines():
                ...
                self.event_list[json_data['event_type']] = []
        print(self.role_dic)
    ...
    def get_trigger(self, sentence):
        ...
        with torch.no_grad():
            ...
            while idx < len(pred):
                if pred[idx] % 2 == 1:
                    _type = pred[idx]
                    event_type_list.append(self.id2label[_type].split('B-')[1])
                    ...
                    end_list.append(end)
                idx += 1
        return event_type_list, start_list, end_list

    def get_role(self, sentence, event_type_list, start_list, end_list, news_list_idx):
        ...
        # event_type_list = ['人生-产子/女']
        # start_list = [120]
        # end_list = [122]

        self.role_model.eval()
        with torch.no_grad():
            for index, event in enumerate(event_type_list):

                answer = [news_list_idx]
                ...
                for role in self.role_dic[event]:
                    ...
                    while idx < len(pred):
                        if pred[idx] == 1:
                            ...
                            if sentence[start:end] not in ans:
                                ans.append(sentence[start:end])
                        idx += 1
                    ans = ','.join(ans)
                    answer.append(ans)
                if len(set(answer)) != 2 and answer not in self.event_list[event]:
                    self.event_list[event].append(answer)

    def get_event_info(self):
        for k, _ in self.event_list.items():
            self.event_list[k] = []

        for news_list_idx, news in enumerate(self.news_list):
            text = news['title'] + '。。。。。。' + news['text']
            event_type_list, start_list, end_list = self.get_trigger(text)
            if len(event_type_list) != 0:
                print(text)
                print(event_type_list, start_list, end_list)
                self.get_role(text, event_type_list, start_list, end_list, news_list_idx)
